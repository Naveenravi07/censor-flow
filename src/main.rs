...
fn main() -> Result<()> {
    ...
    let default_in = host.default_input_device().expect("No default input device available");
    let default_out = host.default_output_device().expect("No default output device available");
    println!(" Default in : {:?}", default_in.name()?);
    println!(" Default out : {:?}", default_out.name()?);

    let mut cfg_in = default_in.supported_input_configs()?;
    let supported_in = cfg_in.next().expect("No supported input config").with_max_sample_rate();
    println!("Max supported in : {:?}", supported_in);

    let mut cfg_out = default_out.supported_output_configs()?;
    let supported_out = cfg_out.next().expect("No supported output config").with_max_sample_rate();
